...
**https://clstudy.online** —— 演示账号 `222222` / `222222`（登录页可点击自动填入）

> 部署在一台 1.6G 内存的阿里云服务器（香港区）上，与另一个项目共存。80/443 端口由宿主机 Nginx 统一入口按域名分流，两个项目的对外地址互不影响。为了塞进去，JVM 参数、容器内存上限、Tomcat 线程数都做过针对性裁剪，详见 [部署上线指南](./docs/05-部署上线指南.md)。
